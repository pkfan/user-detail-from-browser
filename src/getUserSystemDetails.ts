...
export function getUserSystemDetails() {
  const system = window.clientInformation as any
  ...
  const deviceDetailsFromUserAgent = createSystemDataFromUserAgent(
    system.userAgent
  )

  return { ...userSystemDetails, ...deviceDetailsFromUserAgent }
}
